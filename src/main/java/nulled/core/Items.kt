...
import nulled.ext.Item.hasAction
import nulled.ext.Item.name
import nulled.ext.Item.slot
import nulled.ext.Item.widgetID
import java.util.*
...
object Items {
    ...
    fun inventoryContains(id: Int): Boolean {
        val def = API.client.getItemDefinition(id)
        return getAll(id)?.any { it.id == id || def.linkedNoteId == id } ?: false
    }
    ...
    fun equipmentContains(name: String): Boolean {
        return getAll(name, container = InventoryID.EQUIPMENT)?.any {
            it.name == name
        } ?: false
    }
    ...
    fun getAll(vararg names: String, container: InventoryID? = InventoryID.INVENTORY): ArrayList<Item>? {
        return getAll(container!!)?.filter {
    ...
    fun getAll(container: InventoryID = InventoryID.INVENTORY): ArrayList<Item>? {
        ...
        API.client.getItemContainer(container)?.let {
            var slot = 0
