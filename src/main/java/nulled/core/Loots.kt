...
object Loots {
    ...
    fun getAt(worldPoint: WorldPoint) : ArrayList<TileItem>? {
        ...
        API.client.scene.tiles.forEach { z ->
            z?.forEach { x ->
                x?.forEach { y ->
    ...
    fun getFirst(vararg names: String, sortByDistance: Boolean = true): TileItem? {
        return getAll(sortByDistance)?.firstOrNull { names.contains(it.name)}
    }
    ...
    fun exists(name: String): Boolean {
        return getAll()?.any { it.name == name } ?: false
    }
    ...
    fun exists(names: List<String>): Boolean {
        return getAll()?.any { it.name in names } ?: false
    }
